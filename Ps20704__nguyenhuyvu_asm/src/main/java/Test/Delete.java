...
public class Delete {
	...
	public void FillLoginDetails(String sUserName, String sPassword) throws Exception {
		try {
			...
			Thread.sleep(1000);
			WebElement nameTest = driver.findElement(By.className("delete123"));
...
//	}
	...
	public Object[][] Authentication() throws Exception {
		Object[][] testObjArray = ExcelUtils.getTableArray("D:\\Java4\\Ps20704__nguyenhuyvu_asm\\TestDataLogin.xlsx", "Sheet3");
		return (testObjArray);
		...
		try {
			if (result.getStatus() == ITestResult.SUCCESS) {
				TestNGResults.put("1", new Object[] { String.valueOf(rownum),
						"Fill the Delete from data(Username/Password) " + rownum, testU + "/" + testP, "Pass" });
				exportResult(TestNGResults);
			} else if (result.getStatus() == ITestResult.FAILURE) {
				TestNGResults.put("1", new Object[] { String.valueOf(rownum),
						"Fill the Delete from data(Username/Password) " + rownum, testU + "/" + testP, "Fail" });
				TestNGDefects.put("1", new Object[] { String.valueOf(rownumD),
						"Fill the Delete from data(Username/Password) " + rownumD, testU + "/" + testP, "Fail" });
				exportResult(TestNGResults);
